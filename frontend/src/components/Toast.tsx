...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  const schedule = useCallback((id: number, ttl: number) => {
  ...
  const update = useCallback((id: number, kind: ToastKind, message: string, ttl = 4000) => {
    setToasts((t) => t.map((x) => (x.id === id ? { ...x, kind, message } : x)));
    schedule(id, kind === "loading" ? 0 : ttl);
  }, [schedule]);
  ...
  return (
    <ToastCtx.Provider value={api}>
      ...
      <div className="toast-stack" role="status" aria-live="polite">
        <AnimatePresence>
          {toasts.map((t) => (
            <motion.div key={t.id} className={`toast toast--${t.kind}`} layout
              ...
              <button className="toast-x" onClick={() => dismiss(t.id)} aria-label="Dismiss"><X size={14} /></button>
            </motion.div>
          ))}
        </AnimatePresence>
      </div>
